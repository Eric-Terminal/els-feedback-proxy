...
磁盘去重状态。服务端同时接收旧客户端的 schema v1 和当前客户端的 schema v2，
并用请求版本回显响应；v2 在客户端把 MetricKit 递归调用栈压成带父帧编号与深度的
线性数组，避免异常深度数据在手机上反复递归解码和编码。服务端严格校验 schema、原始 payload SHA-256、时间、应用与平台
...
分析优先使用每个 MetricKit 报告内部的构建号；`captured_build` 保留上传时的
外层构建号，`build_source` 说明归因来源。分组同时区分 Bundle、完整指标与仅
磁盘空间指标，以及 MetricKit 的 TestFlight 标记证据；标记与外层渠道冲突时
会明确保留冲突，不把其他 App 的诊断或不同覆盖范围的数据混为同一组。
直方图支持数组和字典两种桶格式；历史客户端把计数 0/1 写成布尔的情况仅在
已知计数字段还原。Signpost 优先读取 `totalSignpostCount`。

非有限、负数及特定磁盘字段中不合理的 EB 级读数单独写入
`measurement-outliers.csv`，保留来源和原值，不参与资源均值或分位数。
诊断详情逐线程保留深度、采样数和归因标记；只有明确归因的线程进入
`top_frame`，没有该标记时留空。原始文件不被改写。
